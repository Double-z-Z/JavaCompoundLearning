# DevOps Dashboard REST API — 完整使用指南

> **版本**: v2.0 (V1+V2 双轨运行)
> **最后更新**: 2026-05-22
> **基础URL**: http://localhost:8080
> **Swagger UI**: http://localhost:8080/swagger-ui.html
> **状态**: ✅ 双轨运行中 — REST API 与 MCP Server 共存
...
| 维度 | REST API (V1) | MCP Tools (V2) |
|------|--------------|----------------|
| **适用场景** | 前端页面、外部系统集成、CI/CD 脚本 | AI 对话交互、自然语言操作 |
| **协议** | HTTP/REST | MCP Protocol (SSE/HTTP) |
| **认证方式** | Token/Session | MCP Client 认证 |
| **操作粒度** | 单次请求单操作 | AI 可编排多步骤工作流 |
| **目标用户** | 开发者、运维工程师、自动化脚本 | Claude/GPT/Trae 等 AI 助手 |
| **推荐优先级** | 程序化调用首选 | 人机交互首选 |

### ✅ REST API 在 V2 中的定位

**REST API 仍然完全有效，适用于以下场景：**

1. **前端 Web UI 调用** — Dashboard 页面的数据展示和操作
2. **CI/CD 流水线集成** — Jenkins/GitLab CI 自动化环境管理
3. **外部系统对接** — 监控平台、CMDB 等系统调用
4. **脚本批量操作** — Shell/Python 脚本批量创建/销毁环境
5. **调试与测试** — 直接 curl 测试接口行为

**V2 新增能力（仅 MCP 提供）：**
- 主机拓扑发现 (`hosts://topology`)
- 服务模板查询 (`templates://list`)
- 远程压测执行 (`test_load`)
- 网络路径分析 (`analyze_network_path`)
- 实验会话管理 (`session_*`)
...
> ✅ **系统自动填充了默认值！**
>
> 💡 **V2 提示**：如果你在使用 AI 助手（如 Claude/Trae），也可以通过 MCP Tool `env_create` 完成相同操作，且能自动获取主机拓扑信息。
...
>
> 🆕 **V2 增强**：通过 MCP `hosts://topology` 可查询主机实时资源余量，智能推荐配额。
...
🆕 **V2 增强**：通过 MCP `hosts://topology` 可动态查询可用节点列表，AI 可智能推荐部署位置。
...
- ✅ 环境必须存在
- ✅ 环境不能已是 DESTROYED 状态
- ⚠️ 如果环境是 RUNNING 状态，会先尝试 STOP 再 DESTROYED
...
当前版本（v2.0）保留了 V1 的完整 REST API 能力，同时新增以下 MCP 专属功能：

| 功能 | 版本 | 接口方式 | 说明 |
|------|------|---------|------|
| 🖥️ 主机拓扑发现 | v2.0 | MCP Resource | `hosts://topology` 返回 PVE/VM 层次结构 |
| 📦 服务模板库 | v2.0 | MCP Resource | `templates://list` 查询可用服务模板 |
| 🧪 远程压测 | v2.0 | MCP Tool | `test_load` 支持 wrk/hey/ab |
| 🔍 网络路径分析 | v2.0 | MCP Tool | `analyze_network_path` 判断压测可信度 |
| 📝 实验会话 | v2.0 | MCP Tool | `session_create/conclude` 全生命周期管理 |
| 🐳 Docker Compose 集成 | v1.1 | REST API | 创建环境时实际启动容器 |
| 📦 服务部署 | v1.2 | REST API | 向环境添加 Nacos/MySQL等服务 |
| 📊 实时监控 | v1.3 | REST API | 查看 CPU/内存使用率 |
| 📝 日志流 | v1.4 | REST API | 实时查看容器日志 |
...
- [MCP Tools 完整定义](./02-MCP-Tools.md) — V2 新增的 MCP 协议接口
- [Java 接口定义](./03-Java-接口定义.md) — 后端 Java Interface 契约
