...
public class MessageService {
    ...
    public MessageService(SessionManager sessionManager) {
        this(sessionManager, new MessageStore());
    }

    public MessageService(SessionManager sessionManager, MessageStore messageStore) {
     ...
     * 处理历史消息请求
     */
    public void handleHistoryRequest(Channel channel, HistoryRequestMessage request) {
        String userId = sessionManager.getUserId(channel);
        if (userId == null) {
            logger.warn("Unauthenticated user tried to request history");
            return;
        }
        ...
        List<Message> messages = messageStore.getRecentMessages(count);
        HistoryResponseMessage response = new HistoryResponseMessage(messages);

        channel.writeAndFlush(response);
        logger.info("History sent to {}, count: {}/{}, store size: {}",
            userId, messages.size(), count, messageStore.size());
    }
