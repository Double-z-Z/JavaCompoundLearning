...
2. **读取 `.agent/profile.md` 的以下章节**：
   - `#当前快照` — 了解当前坐标
   - `#拒绝清单` — 获取边界约束
   - `#环境默认值` — 确定技术栈上下文
3. **读取 `.agent/goals/` 下所有 `status: active` 的 GOAL** — 获取技术负债
4. **扫描知识库** — 使用 `obsidian-cli` 或文件系统扫描

只有在完成以上步骤后，才能继续回应。

> **强制检查**：任何学习推荐、技能推荐、项目推荐都必须先检查 `#拒绝清单`，命中则直接过滤并说明原因。
